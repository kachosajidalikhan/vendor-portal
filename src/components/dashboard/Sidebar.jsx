import { X, LogOut } from "lucide-react"; // Import X icon
...
export default function Sidebar({ onLinkClick, onClose }) {
  ...
  if (!isSidebarOpen) return null; // Sidebar band hai to render mat karo
    ...
    return (
    <div ref={sidebarRef} className="lg:w-68 ml-6 lg:ml-0 bg-white flex flex-col relative shadow-lg h-screen z-50">
      ...
      <div className="px-4 pb-4 lg:hidden block">
        <button onClick={onClose}>
          <X className="w-6 h-6 text-gray-600 hover:text-pink-600" />
        </button>
      </div>
      </div>

      {/* Navigation */}
      <nav className="flex-1 p-4 mt-5 overflow-y-auto">
        <ul className="space-y-2">
          ...
          <li className="relative">
            <NavLink
              ...
              onClick={onLinkClick}
              className={({ isActive }) =>
                `flex items-center text-sm lg:text-lg gap-3 p-2 rounded-md font-medium transition-colors duration-150 ${isActive
                  ? "text-pink-600"
                  : "text-gray-500 hover:bg-pink-50"
                }`
              }
